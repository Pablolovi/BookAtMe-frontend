...
export const updateUserProfile = async (userData) => {
  try {
    const response = await api.put('/api/users/profile', userData);  // La ruta puede cambiar según tu API
...
export const getProfile = async () => {
    try {
      const response = await api.get('/api/users/profile');  // Llamada a la API para obtener los datos del perfil del usuario
